 ...
 * Command greaterCommand = new LessThanCommand();
 * Command lessCommand = new LessThanCommand();
 *
 * double firstVal = 50;
 ...
 *  * lessCommand.addChild(firstConstant);
 *
 * double greaterThan = greaterCommand.execute(turtle, globalParams); // this will be 0 in this example
 * double lessThan = lessCommand.execute(turtle, globalParams); // this will be 1 in this example
 * ...
 *
 * A thing to note when using LessThanCommand.java is that when it is time to compare, .execute should be called
